...
public class servletDispatch extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	// with forward method response will be from the second servlet
    	
    	//request.getServletContext().getRequestDispatcher("/servletDispatch1").forward(request, response);
    	
    	//with include method response will be from this servlet only
    	request.getServletContext().getRequestDispatcher("/servletDispatch1").include(request, response);
    	response.sendRedirect("/servletRequestDispatch/first.html");
    	
    	// with forward below statement is executed
